const loadDataButton = document.getElementById("loader") as HTMLButtonElement
const tableToggle = document.getElementById("tableToggle") as HTMLSelectElement
const table = document.getElementById("table") as HTMLTableElement
const modifyToggle = document.getElementById("modifyToggle") as HTMLSelectElement
...
let checkedRow: HTMLTableRowElement;

type tablePostBody = {
    tableToLoadName: string
};
 ...
 */
function getAndUpdateDropDown(): void {
    fetch('http://localhost:4567/table-names')
        .then((response) => response.json())
        .then((t: tableNames) => updateTableNamesDropDown(t.tableNames))
 ...
 */
function updateTableNamesDropDown(tableNames: string[]): void {
 ...
 */
function postAndDisplayTable(): void {
    const postParameters: tablePostBody = {
    ...
    fetch('http://localhost:4567/table-data', {
        ...
        .then((response) => response.json())
        .then((t: tableData) => displayTable(t))
        .catch((error: any) => console.error("Error: ", error))
}
 ...
 */
function displayTable(tableContents: tableData): void {
    table.innerHTML = "" // clears the table contents
    const columnNames: string[] = tableContents.columnNames
    const rowContents: string[] = tableContents.rowContents
 ...
 */
function updateTextBoxes(): void {
    const updateInputs = document.getElementById("modifyInputs") as HTMLDivElement
...
function modifyTable(): void {
    switch (modifyToggle.value) {
        case "insert":
            const insertContents = document.getElementById("insertContents") as HTMLInputElement
            const insertBody: insertPostBody = {
            ...
            break;
        case "delete":
            const deleteBody: deletePostBody = {
                tableName: tableToggle.value,
            ...
            break;
        case "update":
            const columnToDelete = document.getElementById("columnName") as HTMLInputElement
