...
export default function IntakePage() {
    ...
    window.onload = () => {
        ...
        axios.post('http://localhost:4567/generic-table-data', { table_name: 'Provider'})
            .then(response => {
                for (const provider in response.data) {
                    // @ts-ignore
                    const fullName = response.data[provider]['first_name'] + ' ' + response.data[provider]['last_name'];
                                ...
                                <form onSubmit={event => {handleSubmit(event)}}>
                                    ...
                                    <div className="row">
                                        <div className="col-md-4 mb-4">
                                            ...
                                            <select
                                                className="patient-intake-select"
                                                id="provider-select"
                                                onChange={(e) => {setProvider_id(e.target.value); console.log(provider_id)}}
                                            >
